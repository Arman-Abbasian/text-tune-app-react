    ...
    <div className={`w-full ${className}`}>
      ...
      >
        {tags.map((tag, index) => (
          <div
            key={index}
            className="inline-flex items-center gap-1 rounded-md bg-primary-500 px-2 py-1 text-xs text-primary-100"
          >
            <span>{tag}</span>
            <button
              type="button"
        ...
        <input
          ...
          name={name}
          className="flex-1 border-0 bg-transparent p-0 placeholder:text-primary-700/50 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 text-primary-900"
        />
      </div>
    </div>
  )
